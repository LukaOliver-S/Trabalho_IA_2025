    ...
    def __init__(self):
      ...
      #  self.lidar_global.enablePointCloud() 
        self.lidar_gps = LidarGpsController(
            ...
            max_range=5.5,
            debug=self.DEBUG,
            yaw_provider=self._compass_yaw_vector,
            enable_logging=True,
            ground_truth_provider=self.get_ground_truth, #apenas para validação e apresentação!!!
        )
        ...
        self.fuzzy = FuzzySimple(v_max=0.09)
        # ================= PARÂMETROS =================
    ...
    # ================= AVOID ====================
     ...
     # ================== OBJECT DETECTOR==========
        ...
        self.mission_controller = MissionController(self.navigator, self)
         ...
         # GPS device (apenas para validação!!!!!!!!)
        self.gps = self.robot.getDevice('gps')  
        if self.gps:
            self.gps.enable(self.time_step)

    def get_ground_truth(self):
            """Retorna posição real do GPS (APENAS para análise)"""
            if self.gps:
